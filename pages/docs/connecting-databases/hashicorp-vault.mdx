      ...
      uri:
        valueFrom:
          vault:
            ...
            connectionTemplate: postgres://{}:{}@postgres:5432/my-db
...
```
        vault.hashicorp.com/agent-inject-template-my-db: |
          ...
          postgres://{}:{}@postgres:5432/my-db
          {{- end }}
```
where `my-db` is the configured secret and `postgres://{}:{}@postgres:5432/my-db` is the configured connectionTemplate.
